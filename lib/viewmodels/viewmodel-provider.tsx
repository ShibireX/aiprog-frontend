...
export function ViewModelProvider({ children }: { children: ReactNode }) {
  ...
  const authViewModel = useRef(new AuthViewModel(authState, wrappedSetAuthState));
  const searchViewModel = useRef(new SearchViewModel(searchState, wrappedSetSearchState, authViewModel.current));
  const dashboardViewModel = useRef(new DashboardViewModel(dashboardState, wrappedSetDashboardState, authViewModel.current));
  ...
  searchViewModel.current = new SearchViewModel(searchState, wrappedSetSearchState, authViewModel.current);
  dashboardViewModel.current = new DashboardViewModel(dashboardState, wrappedSetDashboardState, authViewModel.current);
  ...
  useEffect(() => {
    ...
    if (searchFolderHash !== lastSearchFolderHash.current && 
    ...
    else if (dashboardFolderHash !== lastDashboardFolderHash.current && 
