...
export class AuthFormViewModel {
  ...
  constructor(initialState: AuthState, setState: (state: AuthState) => void, router?: ReturnType<typeof useRouter>) {
  ...
  get fieldErrors() { return this.state.fieldErrors; }
  ...
  private validateRepeatPassword = (password: string, repeatPassword: string): string | undefined => {
  ...
  private validateForm = (): boolean => {
    ...
    if (this.state.mode === 'signup') {
      ...
      errors.repeatPassword = this.validateRepeatPassword(this.state.password, this.state.repeatPassword);
    }
  ...
  onSubmit = async () => {
    ...
    try {
      ...
      this.updateState({
        isSubmitting: false,
        errorMessage: err.message || `${this.state.mode === 'signup' ? 'Signup' : 'Login'} failed`,
      });
    }
  };
  ...
  private loginUserAPI = async (email: string, password: string): Promise<LoginResponse> => {
    const variables = { 
